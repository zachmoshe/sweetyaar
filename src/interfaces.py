...
class BluetoothInterface:
    ...
    _UUID_CHAR_CURRNET_PLAYLIST = bluetooth.UUID("10000004-2504-2021-0000-000079616172")
    ...
    _UUID_CHAR_PLAYLISTS = bluetooth.UUID("10000007-2504-2021-0000-000079616172")
    ...
    def __init__(self, cfg):
        ...
        self.current_playlist_char = aioble.Characteristic(self.sweetyaar_service, self._UUID_CHAR_CURRNET_PLAYLIST, notify=True, read=True, write=True)
        ...
        self.playlists_char = aioble.Characteristic(self.sweetyaar_service, self._UUID_CHAR_PLAYLISTS, notify=True, read=True)
    ...
    def handle_controller_event(self, event):
        ...
        if "change_playlist" in event:
            self.current_playlist_char.write(event["change_playlist"].encode("utf8"), send_update=True)
        if "volume" in event:
        ...
        if "update_playlists" in event:
            playlists_str = json.dumps(event["update_playlists"])
            self.playlists_char.write(playlists_str.encode("utf8"))
    ...
    async def listen(self, actions_callback):
        ...
        playlist_service = asyncio.create_task(self._run_playlist_service(actions_callback))

    async def _run_playlist_service(self, actions_callback):
        while True:
            await self.current_playlist_char.written()
            playlist_name = self.current_playlist_char.read().decode("utf8")
            actions_callback(controller.Actions.CHANGE_PLAYLIST, playlist_name)
